...
export async function servePage(req, res) {
    if (req.url == '/tests/flashpage.js' || req.url == '/tests/instantpage.js') {
        return serveFlashpageScript(res)
    }
...
async function serveFlashpageScript(res) {
    const path = new URL('../flashpage.js', import.meta.url)
